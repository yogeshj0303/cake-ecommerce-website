  ...
  <form action="<?php echo e(route('nomination.updateStatus', $nomination->nid)); ?>" id="nomination-status-form" method="POST">
    ...
    <div class="row">
        <div class="col-md-12 mb-3">
            ...
            <!-- Approve Button -->
              <button type="button" id="send-button" class="btn btn-primary me-2" data-docuser="<?php echo e($nomination->nid); ?>" data-bs-toggle="modal" data-bs-target="#ForwardModel" <?php if($nomination->witness_first_status === 'pending' || $nomination->witness_second_status === 'pending'): ?> disabled <?php endif; ?>>Send</button>

              
            <button type="button" id="approve-button" class="btn btn-success me-2" data-docuser ="<?php echo e($nomination->nid); ?>" <?php if($nomination->witness_first_status === 'pending' || $nomination->witness_second_status === 'pending'): ?> disabled <?php endif; ?> >Send & Sign</button>

            <!-- Reject Button -->
            <button type="button" id="reject-button" class="btn btn-danger" <?php if($nomination->witness_first_status === 'pending' || $nomination->witness_second_status === 'pending'): ?> disabled <?php endif; ?>>Reject</button>
            ...
            <button type="button"  class="btn btn-danger"  >Rejected </button>
              <?php endif; ?>
               <?php if($nomination->witness_first_status == 'pending' && ($nomination->sstatus === 'pending') && $nomination->witness_second_status === 'pending'): ?>
              <button type="button" id="sendWitnessOtp-button" class="btn btn-primary me-2" data-docuser="<?php echo e($nomination->nid); ?>"  data-bs-toggle="modal" data-bs-target="#sendWitnessOtp">Verify First Witness Digital Signature</button>
               <button type="button" class="btn btn-primary me-2" disabled>Verify Second Witness Digital Signature</button>
              <?php elseif($nomination->witness_first_status === 'approved' && ($nomination->sstatus ==='pending') && $nomination->witness_second_status === 'pending' ): ?>
          <button type="button" class="btn btn-success me-2" disabled>Witness First (verified)</button>
              <button type="button" id="sendWitnessOtp-button" class="btn btn-primary me-2" data-docuser="<?php echo e($nomination->nid); ?>" data-bs-toggle="modal" data-bs-target="#sendWitnessOtp">Verify Second Witness Digital Signature</button>
               <?php elseif($nomination->witness_first_status === 'approved' &&  $nomination->witness_second_status === 'approved'  ): ?>
          <button type="button" class="btn btn-success me-2" disabled>Witness First (verified)</button>
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                <button type="button" id="verify-button" class="btn btn-danger"  data-docuser="<?php echo e($nomination->nid); ?>" 
                 <?php if($nomination->witness_first_status === 'pending'  ): ?> data-witness_type="first" <?php else: ?> data-witness_type="second" <?php endif; ?> >Send</button>
            </div>
        </div>
    </div>
</div>       
...
<script src="<?php echo e(URL::asset('build/js/pages/select2.init.js')); ?>"></script>
 ...
 <?php if($nomination->witness_first_status == 'pending' || $nomination->witness_second_status == 'pending'): ?>
<script>
        ...
        .then(data => {
            if (data.success) {
                ...
                verifyUserClerkModal.hide(); // Close the modal on success
                    window.location.href = "<?php echo e(route('nomination_pending')); ?>";
            } else {
            ...
            success: function(response) {
                if (response.success) {
                    $('#ForwardModel').modal('hide'); // Close the modal
                            window.location.href = "<?php echo e(route('nomination-index')); ?>";
...
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.master', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/u532655452/domains/eofficess.com/public_html/resources/views/nomination/viewnomination.blade.php ENDPATH**/ ?>