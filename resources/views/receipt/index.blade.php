            ...
            <div class="card-body">
                <div class="listjs-table" id="customerList">
                    ...
                    @if((isset($permissions)) && ( ($permissions['other_receipt_create'] == 2)))
                      ...
                      <a href="{{route('receipts.create')}}"><button type="button" class="btn btn-success ">Add </button></a>
                      @endif
                    <a href="{{route('receipts.pending')}}"><button type="button" class="btn btn-warning ">Request </button></a>
                <a href="{{route('receipts.history')}}"><button type="button" class="btn btn-primary ">History </button></a>
...
<td>
    {{ $usernameFrwd ? $usernameFrwd->first_name . ' ' . $usernameFrwd->middle_name . ' ' . $usernameFrwd->last_name . '-' . $usernameFrwd->department_name . '-' . $usernameFrwd->designation_name : 'N/A' }}
                                     ...
                                     <td>{{$receipt->reject_description}}</td>
                                    ...
                                    <!-- Image placed over the text -->
                                                <span class="badge bg-success-subtle text-success text-uppercase">Approved</span>
                                </div>@elseif($receipt->status == 'rejected' && $receipt->frwd_hod_id == null )
...
@if($receipt->status != 'rejected' && $receipt->status != 'approved_clerk' && $receipt->status != 'approved' )
 @if((isset($permissions)) && (($permissions['other_receipt_edit'] == 2) ))
                                    <a href="{{route('receipts.edit' ,$receipt->id )}}" class="btn btn-primary btn-sm">Edit</a>
                                    @endif
                                        @endif
                                    @endif
                                     @if((isset($permissions)) && (($permissions['other_receipt_delete'] == 2) ))
<button class="btn btn-sm btn-danger remove-item-btn" data-bs-toggle="modal" data-id="{{ $receipt->id }}" data-bs-target="#deleteRecordModal">
    Remove
</button>
@endif

                        
                                                                                                                       <button class="btn btn-sm btn-warning remove-item-btn" onclick="printAffidavit({{ $receipt->id }})">
    Print
</button>
