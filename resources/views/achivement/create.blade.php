...
@endphp
    ...
    <div class="col-lg-12">
        <div class="card">
            ...
            <div class="card-body">
<form class="leave-form" method="POST" id="achivement" action="{{ route('storeachievement') }}" autocomplete="off" enctype="multipart/form-data">
                    @csrf
...
<select id="name" name="name" class="form-control @error('name') is-invalid @enderror">
    ...
    <select name="achivement_name" id="achivementName" class="form-select mb-3">
        ...
        @foreach($achivements as $achivement)
            <option value="{{ $achivement->id }}">{{ $achivement->name }}</option>
        @endforeach
    </select>
    @error('achivement_name') <!-- Corrected error variable -->
    <div class="invalid-feedback" style="color: red;">
        {{ $message }}
    </div>
    @enderror
