...
<tr class="table-row">
    ...
    <td class="table-cell">{{ $user->last_working_date ?? '- -' }}</td>
</tr>

<tr class="table-row">
    <td class="table-cell">{{ $rowNumber++ }}</td>
    <td class="table-cell" style="font-weight: bold;">Transfer Location </td>
    <td class="table-cell">{{ $user->transfer_state ?? '- -' }} - {{ $user->transfer_district ?? '- -' }} 
        {{ $user->transfer_taluka ? '- ' . $user->transfer_taluka : '' }}
