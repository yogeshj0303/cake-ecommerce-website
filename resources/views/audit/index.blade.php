    ...
    <div class="col-lg-12">
        <div class="card">
            ...
            <div class="col-sm-auto">
                                <div>
                                   <button type="button" class="btn btn-success add-btn" onclick="window.location.href='{{ route('audits.create') }}'">
    Add
</button>

                         <button type="button" class="btn btn-warning add-btn" onclick="window.location.href='{{ route('pending-request') }}'">
    Request
</button>

                               <button type="button" class="btn btn-primary" onclick="window.location.href='{{ route('showauditHistory') }}'">History</button>
        ...
        <th>Designation Name</th>
                        ...
                        <tbody>
                            @foreach($audits as $audit)
                              <tr id="userRow{{ $audit->id }}">
                                                                <td>{{ $audit->id }}</td>
                                 <td>{{ $audit->first_name }}  {{ $audit->middle_name }} {{ $audit->last_name }}</td>

                                <td>{{ $audit->state }}</td>
                                <td>{{ $audit->district }}</td>
                                <td>{{ $audit->taluka }}</td>
                                <td>{{ $audit->org_name }}</td>
                                <td>{{ $audit->department_name }}</td>
                                <td>{{ $audit->designation_name }}</td>
                ...
                <button class="btn btn-info btn-sm view-description" 
                    ...
                    <i class="fas fa-eye"></i>
                                ...
                                <td>{{ $audit->report_view }}</td>
                                <td>{{ $audit->audit_remark }}</td>
                                <td>{{ $audit->auditor_name }}</td>
                                <td>{{ $audit->position }}</td>
                                <td>{{ $audit->organisation_name }}</td>
                                <td>{{ $audit->auditor_veri_des }}</td>
...
$usernameFrwd = DB::table('users')
    ...
    ->where('users.id', $audit->frwd_hod_id) 
    ->first();
...
<td>
          ...
          <td class="status">
                                            ...
                                            @if ($audit->status == 'pending')
                                                <span class="badge bg-warning-subtle text-warning text-uppercase">Pending</span>
                                                 @elseif ($audit->status == 'approved_clerk')
                                                ...
                                                @elseif ($audit->status == 'rejected')
                                                 <span class="badge bg-danger-subtle text-danger text-uppercase">Rejected</span>
                                                   @elseif ($audit->status == 'forward')
                                                 <span class="badge bg-danger-subtle text-danger text-uppercase">forward</span>

                                                @endif
                                      </td>
                                       <td>@if($audit->clerk_otp_status == 1)  <div class="checkmark-container">
                                  
                                
                                    <!-- Image placed over the text -->
                                                <span class="badge bg-success-subtle text-success text-uppercase">Approved</span>
                                </div>@else @if($audit->status == 'rejected')  
                                ...
                                @endif @endif</td>
                               <td>@if($audit->hod_otp_status == 1 ) <div class="checkmark-container">
                                  
                                
                                    <!-- Image placed over the text -->
                                                <span class="badge bg-success-subtle text-success text-uppercase">Approved</span>
                                </div>  @else @if($audit->status == 'rejected')  <div class="checkmark-container">
                             ...
                             <td>{{ $audit->reason_description }}</td>
    ...
    <div class="d-flex align-items-center">
        ...
        <div class="d-flex gap-2">
             @if ($audit->status != 'rejected'  && $audit->status != 'approved_clerk'&& $audit->status != 'approved')
<button class="btn btn-sm btn-primary" onclick="window.location.href='{{ route('audits.edit', $audit->id) }}'">Edit</button>
            @endif
<button class="btn btn-sm btn-success" onclick="window.location.href='{{ route('audits.show', $audit->id) }}'">View</button>
         <button class="btn btn-sm btn-danger remove-item-btn" data-bs-toggle="modal" data-bs-target="#deleteRecordModal" data-id="{{ $audit->id }}">Remove</button>
                                                                                                                                         <button class="btn btn-sm btn-warning remove-item-btn" onclick="printAffidavit({{ $audit->id }})">
    Print
</button>
        </div>
   ...
   <script>
    function printAffidavit(userId) {
        var printWindow = window.open('/auditprint/' + userId, '_blank');
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        @foreach($audits as $audit)
            const dropdown{{ $audit->id }} = document.getElementById(`dropdown-{{ $audit->id }}`);
            checkStatusAndHide(dropdown{{ $audit->id }}, '{{ $audit->status }}');
        @endforeach
