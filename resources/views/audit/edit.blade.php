...
<iframe id="pdfViewer" style="width:100%; height:500px;" frameborder="0"></iframe>
                    ...
                    <div class="row mb-3">
                        <div class="col-md-4">
                            ...
                            <select id="state" name="state" class="form-control" required>
                                ...
                                @foreach($statesData['states'] as $state)
                                    <option value="{{ $state['state'] }}" {{ $audit->state === $state['state'] ? 'selected' : '' }}>
                                        {{ $state['state'] }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-4">
                            ...
                            <select id="district" name="district" class="form-control" required>
                                <option value="{{ $audit->district }}">{{ $audit->district }}</option>
                            </select>
                            @error('district')
                                <div class="invalid-feedback text-red">{{ $message }}</div>
                            @enderror
                    ...
                    <div class="row mb-3">
                        ...
                        <div class="col-md-4">
                            ...
                            <select id="taluka-field" name="taluka" class="form-control" >
                                <option value="{{ $audit->taluka }}">{{ $audit->taluka }}</option>
                            </select>
                            @error('taluka')
                                <div class="invalid-feedback text-red">{{ $message }}</div>
                            @enderror
    ...
    <option value="{{ $audit->user_id }}">{{ $audit->first_name }}</option>
</select>
<input type="hidden" name="user_id" value="{{ $audit->user_id }}">
...
@error('profile_name')
                    ...
                    <div class="row align-items-center g-3">
                        <div class="col-lg-4">
                            ...
                            <input type="text" id="audit_name" class="form-control" name="audit_name" value="{{ old('audit_name', $audit->audit_name) }}">
                        </div>
                        <div class="col-lg-4">
                            <label class="form-label" for="description">Description</label>
                            <input type="text" id="description" class="form-control" name="description" value="{{ old('description', $audit->description) }}">
                        </div>
                        <div class="col-lg-4">
                            <label for="audit_remark" class="form-label">Audit Remark</label>
                            <input type="text" id="audit_remark" class="form-control" name="audit_remark" value="{{ old('audit_remark', $audit->audit_remark) }}">
                        </div>
                    </div>
                    ...
                    <div class="row align-items-center g-3">
                        <div class="col-lg-4">
                            ...
                            <input type="text" id="auditor_name" class="form-control" name="auditor_name" value="{{ old('auditor_name', $audit->auditor_name) }}">
                        </div>
                        <div class="col-lg-4">
                            <label class="form-label" for="position">Position</label>
                            <input type="text" id="position" class="form-control" name="position" value="{{ old('position', $audit->position) }}">
                        </div>
                        <div class="col-lg-4">
...
<option value="{{ $organisation->id }}" {{ $organisation->org_name === $audit->org_name_by_name ? 'selected' : '' }}>
                    ...
                    <div class="row align-items-center g-3">
                        <div class="col-lg-6">
                            ...
                            <input type="text" id="auditor_veri_des" class="form-control" name="auditor_veri_des" value="{{ old('auditor_veri_des', $audit->auditor_veri_des) }}">
                        </div>
                    </div>
<!--<div class="row">-->
...
<!--            <option value="rejected" {{ $audit->status === 'rejected' ? 'selected' : '' }}>Rejected</option>-->
<!--            <option value="wrong_information" {{ $audit->status === 'wrong_information' ? 'selected' : '' }}>Wrong Information</option>-->
<!--        </select>-->
...
<div class="row" id="reject-description-row" style="display: none;">
    <div class="col-md-12">
        ...
        <textarea class="form-control @error('reject_description') is-invalid @enderror" id="reason_description" name="reject_description">{{ old('reject_description', $audit->reason_description) }}</textarea>
...
<!-- end row -->
    ...
    <div class="hstack gap-2">
        <button type="button" class="btn btn-primary" onclick="window.location.href='{{ route('audits.index')}}'">Back</button>
    </div>
</div>
                </form>
            </div>
...
$(document).ready(function() {
    ...
    // Set initial values
    var initialState = '{{ $audit->state }}';
    var initialDistrict = '{{ $audit->district }}';
    var initialTaluka = '{{ $audit->taluka }}';
    var initialDesignation = '{{ $audit->designation_name }}';
    var initialDepartment = '{{ $audit->name }}';
    var initialOrganisation = '{{ $audit->org_name }}';
        ...
        if (state && district) {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#organisation').val('{{ $audit->org_id }}').trigger('change');
                },
                error: function(xhr) {
...
function loadDepartments() {
    ...
    console.log('Initial Department ID:', '{{ $audit->depart_id }}');
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (Array.isArray(response)) {
                    ...
                    if (response.some(dept => dept.id == '{{ $audit->depart_id }}')) {
                        departmentDropdown.val('{{ $audit->depart_id }}').trigger('change');
                        console.log('Department value set:', '{{ $audit->depart_id }}');
                    } else {
        ...
        if (taluka) {
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $('#designation').val('{{ $audit->design_id }}').trigger('change');
                },
                error: function(xhr) {
