...
<meta name="csrf-token" content="{{ csrf_token() }}">
            ...
            <a href="{{ url()->previous() }}"><button type="button" style="margin: -49px 33px; float:right "  class="btn btn-primary ">Back </button></a>
...
@if($salary->status == 'pending' || ($salary->status == 'approved_clerk' &&  $role->role_name == 'HOD' ))
   ...
   <div class="row" id="greenNoteField_letter" style="margin-top: 0px;">
    <div class="form-group col-md-12">
        <div id="memo-editors" class="memo-editors" contenteditable="true" 
             ...
             overflow-y: auto; padding: 10px; overflow-x: hidden;">
            {!! isset($letter) && $letter->note ? $letter->note : '' !!}
        </div>
...
<!--</div>-->
    ...
    <div class="row mb-3">
        ...
        <div class="col-md-6">
            ...
            <input type="" id="joining_date" name="joining_date" class="form-control" value="{{ $salary->joining_date }}" readonly>
        </div>
    </div>
    ...
    <div class="row mb-3">
        <div class="col-md-4">
         ...
         <div class="col-md-4">
            ...
            <input type="text" id="merge_amount" name="merge_amount" class="form-control" value="{{ $salary->merge_amount }}" readonly>
        </div>
                <div class="col-md-4">
            <label for="direct_added_amount" class="form-label">Direct Add Total Salary</label>
            <input type="text" id="direct_total_salary" name="direct_total_salary" class="form-control" value="{{ $salary->direct_total_salary }}" readonly>
        </div>
  ...
  <div class="row mb-3">
        ...
        <a href="{{ $salary->reference_document ? asset('uploads/'.$salary->reference_document) : '#' }}" 
          ...
          <div class="row mb-3">
            ...
            <div class="col-md-12 mb-3">
                ...
                <form action="{{ route('salary-update-status', $salary->id) }}" id="achievement-form" method="POST">
    ...
    <div class="row">
        <div class="col-md-12 mb-3">
            @if(($salary->status =='pending' || ($salary->status == 'approved_clerk' && $salary->frwd_hod_id == Auth::user()->id )) && Auth::user()->is_admin == 'staff' )
          
            <!-- Approve Button -->
              <button type="button" id="send-button" class="btn btn-primary me-2" data-docuser="{{$salary->id}}" >Send</button>

              
            <button type="button" id="approve-button" class="btn btn-success me-2" data-docuser ="{{$salary->id}}" >Send & Sign</button>

            <!-- Reject Button -->
            <button type="button" id="reject-button" class="btn btn-danger">Reject</button>
            @elseif(($salary->status == 'pending' || $salary->status == 'approved_clerk') && Auth::user()->is_admin != 'staff')
                           ...
                           <div class="form-group" id="frwd_hod" style="display:none">
                                    ...
                                    <select name="hod" class="form-control select2" id="hod_id"  >
                                      ...
                                      @foreach($staffs as $staff)
                                       <option value="{{$staff->id}}" @if($salary->frwd_hod_id ==$staff->id ) selected @endif >{{$staff->first_name}} {{$staff->middle_name}} {{$staff->last_name}}-{{$staff->department_name}}-{{$staff->designation_name}}</option>
                                      @endforeach
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form id="frwd_to_staff_Form"> 
                    <div class="form-group">
                        ...
                        <input type="hidden" name="frwd_leave_id" value="{{ $salary->id }}" id="frwd_leave_id">
                        <label for="frwd_hod_id">Forward To Staff</label>
                        <select name="frwd_hod_id" class="form-control select2" id="frwd_hod_id">
                            @foreach($staffs as $staff)
                                       <option value="{{$staff->id}}" value="{{$staff->id}}" @if($salary->frwd_hod_id ==$staff->id ) selected @endif>{{$staff->first_name}} {{$staff->middle_name}} {{$staff->last_name}}-{{$staff->department_name}}-{{$staff->designation_name}}</option>
                            @endforeach
                        </select>
                    </div>
                </form> 
            ...
            <div class="modal-footer">
              ...
              <!--      <button type="button" class="btn btn-success" id="approveButton" {{ $salary->status != 'pending' ? 'disabled' : '' }} name="status">-->
              <!--          Approve-->
              <!--      </button>-->
              <!--      @elseif( $salary->status == 'rejected')-->
              <!--      <button type="button" class="btn btn-danger" id="rejectButton" {{ $salary->status != 'pending' ? 'disabled' : '' }} name="status">-->
              <!--          Reject-->
              <!--      </button>-->
              <!--      @else-->
              <!--       <button type="button" class="btn btn-success" id="approveButton" {{ $salary->status != 'pending' ? 'disabled' : '' }} name="status" value="approved">-->
              <!--          Approve-->
              <!--      </button>-->
                   
              <!--      <button type="button" class="btn btn-danger" id="rejectButton" {{ $salary->status != 'pending' ? 'disabled' : '' }} name="status" value="rejected">-->
              <!--          Reject-->
              <!--      </button>-->
              <!--      @endif-->
              <!--</div>-->
...
$(document).ready(function() {
    ...
    @if(Auth::user()->is_admin != 'admin' ||  Auth::user()->is_admin == 'admin')
  <?php    if (isset($letter)) :  ?>
