...
@section('content')
 <meta name="csrf-token" content="{{ csrf_token() }}">
    ...
    ($permissions['role_create'] == 2) 
                                   ...
                                   <a href="{{route('roles.create')}}"<button type="button" class="btn btn-success add-btn">
    Add
</button></a>
@endif
  <a href="{{route('roles-history')}}"<button type="button" class="btn btn-primary">
    History
</button></a>
    ...
    ($permissions['role_delete'] == 2)
        ...
        <tr id="userRow{{ $role->id }}">
            
                                        <td class="id">{{ $role->id }}</td>

            <td class="role_name">{{ isset($role->role_name) ? $role->role_name : '' }}</td>
            <td class="state">{{ isset($role->state_name) ? $role->state_name : '' }}</td>
            <td class="dist">{{ isset($role->district_name) ? $role->district_name : '' }}</td>
            <td class="taluka">{{ isset($role->taluka) ? $role->taluka : '' }}</td>
            <td class="contact">{{ isset($role->designation_name) ? $role->designation_name : '' }}</td>
            <td class="department_name">{{ isset($role->department_name) ? $role->department_name : '' }}</td>
             @if((isset($permissions)) && (
    ...
    ($permissions['role_edit'] == 2) 
                        ...
                        <button type="button" class="btn btn-sm btn-primary edit-item-btn" onclick="window.location.href='{{ route('roles.edit', $role->id) }}'">
                            Edit
                        </button>
                        @endif
                    </div>
    ...
    ($permissions['role_view'] == 2) 
                        ...
                        <button type="button" class="btn btn-sm btn-success " onclick="window.location.href='{{ route('roles.show', $role->id) }}'">
                            View
                        </button>
                        @endif
    ...
    ($permissions['role_delete'] == 2) 
                        ...
                        <button class="btn btn-sm btn-danger remove-item-btn" data-id="{{ $role->id }}">
                            Remove
                        </button>
                         @endif
                    </div>
