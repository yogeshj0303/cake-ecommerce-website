    ...
    <div style="display: flex; gap: 0; margin-left: 5px;">
        ...
        <div class="dropdown" style="position: relative;">
            ...
            <ul class="dropdown-menu" aria-labelledby="draftDropdown">
                <li><a class="dropdown-item" href="{{url('create-draft')}}">Create Draft</a></li>
                <li><a class="dropdown-item" href="{{url('view-draft')}}">View Draft</a></li>
            </ul>
        </div>
        <span style="color: black; margin: 0 10px;font-size:25px;">|</span>
            ...
            @if(isset($files) && count($files) > 0)
                @foreach($files as $file)
                    <tr>
                        ...
                        <td>{{ $file->main_category }}</td>
                        <td>{{ $file->sub_category }}</td>
                        <td>{{ $file->classified }}</td>
                        <td>{{ $file->remarks }}</td>
                        <td>{{ $file->previous_ref }}</td>
                        <td>{{ $file->later_ref }}</td>
                    </tr>
                @endforeach
            @else
                <tr>
